...
output "aws_lb_dns_name" {
  value = aws_lb.tj_lb.dns_name
}

output "aws_lb_zone_id" {
  value = aws_lb.tj_lb.zone_id
}

#### load balancer - resource creation 
resource "aws_lb" "tj_lb" {
  name               = var.lb_name
...
/*
resource "aws_lb_target_group_attachment" "tj_lb_target_group_attachment" {
  target_group_arn = var.lb_target_group_arn
  target_id        = var.ec2_instance_id # Replace with your EC2 instance reference
  port             = var.lb_target_group_attachment_port
}*/

# alb is listening at port 5000
resource "aws_lb_listener" "tj_lb_listner" {
  load_balancer_arn = aws_lb.tj_lb.arn
...
resource "aws_lb_listener" "tj_lb_https_listner" {
  load_balancer_arn = aws_lb.tj_lb.arn
  port              = var.lb_https_listner_port    #port = 443
  protocol          = var.lb_https_listner_protocol
  ssl_policy        = "ELBSecurityPolicy-FS-1-2-Res-2019-08"
  certificate_arn   = var.tj_acm_arn
